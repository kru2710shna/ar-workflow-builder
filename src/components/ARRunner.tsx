    ...
    >
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-8">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {/* Instruction text */}
            <p className={`step-text font-semibold leading-relaxed ${arMode ? "text-primary glow-text" : "text-foreground"}`}>
              {step.text}
            </p>
            ...
            {step.timer > 0 && (
              <div className={`inline-flex items-center gap-3 px-6 py-3 rounded-lg ${
                arMode ? "bg-accent/10 border border-accent/30" : "bg-secondary border border-border"
              }`}>
                <Timer className={`w-6 h-6 ${timeLeft === 0 ? "text-accent" : "text-primary"}`} />
                <span className={`text-3xl font-mono font-bold ${
                  timeLeft === 0 ? "text-accent" : "text-foreground"
                }`}>
                  {formatTime(timeLeft)}
                </span>
                {timeLeft === 0 && (
      ...
      <div className="p-4 flex items-center gap-4">
        <motion.button
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-5 rounded-lg text-lg font-bold transition-all ${
            currentStep === 0
        ...
        <motion.button
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-5 rounded-lg text-lg font-bold transition-all ${
            currentStep === steps.length - 1
              ...
              : "bg-primary text-primary-foreground hover:bg-primary/90 glow-primary active:bg-primary/80"
          }`}
        >
          NEXT
